...
**Live Deployed Website URL:** [`https://akashkumarray.github.io/Flyrank-Internship/`](https://akashkumarray.github.io/Flyrank-Internship/)
...
* **Core Positioning:** Akash Ray — Applied ML Engineer & Search Intelligence Researcher.
* **Integrated Professional Links:**
  * **LinkedIn Profile:** `https://linkedin.com/in/akashkumarray`
  * **GitHub Profile:** `https://github.com/akashkumarray`
...
The internet works the exact same way. Computers communicate using numeric **IP Addresses** (like `185.199.108.153`), but humans remember names like `akashkumarray.github.io`. 
...
When a visitor types `https://akashkumarray.github.io/Flyrank-Internship/` into their browser and hits Enter, a lightning-fast 5-step journey happens in under 50 milliseconds:

```text
Visitor's Browser
       │
       ▼
1. Recursive Resolver (ISP / 1.1.1.1) ──► "Who knows where akashkumarray.github.io lives?"
       │
       ▼
...
3. TLD Nameserver (.io) ──────────────► "I don't know the exact IP, but ask GitHub's Nameservers."
       │
       ▼
4. Authoritative Nameserver ──────────► "Found it! akashkumarray.github.io lives at 185.199.108.153."
       │
       ▼
...
The resolver asks a **Root Nameserver**. There are 13 root server clusters worldwide that direct internet traffic. The root server says: *"I don't know the exact IP for `akashkumarray.github.io`, but it ends in `.io`, so go ask the `.io` Top-Level Domain (TLD) server."*
...
The resolver asks GitHub's **Authoritative Nameserver**. This server holds the actual DNS records for `akashkumarray.github.io`. It looks up the record and answers: *"Yes! `akashkumarray.github.io` is currently hosted at IP address `185.199.108.153`."*
...
* **Example:** Suppose you buy a custom domain `www.akashray.com`. Instead of typing GitHub's IP address (which might change if GitHub updates their servers), you create a CNAME record:
  ```text
  www.akashray.com  ---> CNAME ---> akashkumarray.github.io
  ```
* **Why it matters:** Whenever someone visits `www.akashray.com`, DNS automatically redirects the lookup to `akashkumarray.github.io`. If GitHub changes its server IP addresses under the hood, your custom domain continues working automatically without needing manual updates!
...
4. **`submission/paper_url.txt`:** Pointer text file containing the live published URL (`https://akashkumarray.github.io/Flyrank-Internship/`).
...
|---|---|---|
| **Live HTTPS Site** | PASSED | Live at [https://akashkumarray.github.io/Flyrank-Internship/](https://akashkumarray.github.io/Flyrank-Internship/). |
